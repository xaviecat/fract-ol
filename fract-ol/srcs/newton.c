...
/*   Updated: 2023/01/24 16:07:55 by xcharra          ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
	...
	while (newton->px.x < WIDTH)
	{
		...
		while (newton->px.y < HEIGHT)
		{
			...
			if (i == newton->imax)
				...
				0x00FF0000);
			else if (newton->z.x <= 0.001 * -1 && newton->z.y < 0)
				my_mlx_pixel_put(newton->imgprt, newton->px.x, newton->px.y, \
				0x00a26769 + ((i * 4) << 16)); //0x00a2dcc7 * i / 1000000
			else if (newton->z.x <= 0.001 * -1 && newton->z.y > 0)
				my_mlx_pixel_put(newton->imgprt, newton->px.x, newton->px.y, \
				0x006d2e46 + ((i * 3) << 8)); //0x00a2dcc7 * i / 1000000
			else if (newton->z.y >= 0.001 * -1)
				my_mlx_pixel_put(newton->imgprt, newton->px.x, newton->px.y, \
				0x00d5b9b2 + ((i * 3) << 16)); //0x00a2dcc7 * i / 1000000
			// else
			// 	my_mlx_pixel_put(newton->imgprt, newton->px.x, newton->px.y, \
